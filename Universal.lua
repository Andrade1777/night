...
-- ===== Services seguro com fallback (cole no topo, antes de qualquer uso de game:GetService) =====
local Services = {}
local serviceAliases = {
    ChatService = { "TextChatService", "Chat" },
    -- adicione aqui outros aliases caso precise, ex:
    -- SomeOldName = { "NewServiceName", "AnotherFallback" },
}
...
setmetatable(Services, {
    __index = function(self, key)
        ...
        -- tenta nome exato
        local ok, srv = pcall(function() return game:GetService(key) end)
        if ok and srv then
            rawset(self, key, srv)
            return srv
        end

        -- tenta aliases definidos
        local aliases = serviceAliases[key]
        if aliases then
            for _, name in ipairs(aliases) do
                local ok2, srv2 = pcall(function() return game:GetService(name) end)
                if ok2 and srv2 then
                    rawset(self, key, srv2)
                    return srv2
                end
            end
        end

        -- heurística final para chat
        if key ~= "TextChatService" then
            local ok3, tc = pcall(function() return game:GetService("TextChatService") end)
            if ok3 and tc then rawset(self, key, tc); return tc end
        end
        if key ~= "Chat" then
            local ok4, c = pcall(function() return game:GetService("Chat") end)
            if ok4 and c then rawset(self, key, c); return c end
        end

        warn(("[Services] Serviço solicitado '%s' não encontrado em game:GetService()"):format(tostring(key)))
        rawset(self, key, nil)
        return nil
    end
})

-- expõe globalmente (compatibilidade com outros módulos que usam _G.Services)
_G.Services = _G.Services or Services
-- ============================================================================================
...
local textChatService = _G.Services.TextChatService
